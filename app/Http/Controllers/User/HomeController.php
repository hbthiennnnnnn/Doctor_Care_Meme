...
class HomeController extends Controller
{
    public function home()
    {
        ...
        $departments = Department::where('status', 1)->orderByDesc('id')->get();
         $doctors = Admin::role('Admin')->where('status', 1)->orderByDesc('id')->get();
        $doctors = [];
    ...
    public function book_appointment_page(Request $request)
    {
        ...
        if ($request->has('doctor_id')) {
            $selectedDoctor = Admin::role('Admin')->with('department')->find($request->doctor_id);
            if ($selectedDoctor) {
    ...
    {
        $doctors = Admin::role('Admin')->whereHas('clinic', function ($query) use ($department_id) {
